  ...
  for (i = 0; i < count; i++)
    {
      if (offsets[i] == INVALID_DB_DATA_OFFSET)
        ...
        {
          INFO(" %s%2u: %4x", title, i, offsets[i]);
        }
    }
}
...
{
  INFO("TEAM: %-3s DIV: %3u", team->abbreviation, team->division);
...
{
  INFO("TEAM: %-3s DIV: %3u", stats->abbreviation, stats->division);
...
{
  if (!read_db_file(&db_data->teams, get_db_file(DB_FILE_TEAMS)))
    return FALSE;
  if (!read_db_file(&db_data->carteams, get_db_file(DB_FILE_CARTEAMS)))
    return FALSE;
  return TRUE;
}
                ...
                player_db_data_t *player_data)
{
  ...
  sprintf(new_team->long_name, "New Team Newcomers");
  new_team->division = 1;
  ...
  sprintf(new_team_career->long_name, "New Team Newcomers");
